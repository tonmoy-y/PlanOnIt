...
import { compareAndSwapState, loadState, saveState, STORAGE_KEY, subscribeState } from '../src/persistence';
...
describe('optimistic workspace writes',()=>{
  ...
  it('detects same-version approval conflicts even when timestamps collide',async()=>{const original=initialPlan();saveState(original,[]);const approved={...original,status:'approved' as const,approval:{version:1,providerRevision:0,approvedAt:original.updatedAt}};expect((await compareAndSwapState({plan:original},{plan:approved,activity:[],writerId:'a'})).ok).toBe(true);const conflicting={...original,status:'reservation_failed' as const};const result=await compareAndSwapState({plan:original},{plan:conflicting,activity:[],writerId:'b'});expect(result).toMatchObject({ok:false,error:{code:'CONCURRENT_WRITE_CONFLICT'}});expect(loadState().plan.status).toBe('approved');});
});
