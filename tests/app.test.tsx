...
describe('shared browser workspace',()=>{
  ...
  it('keeps all four navigation destinations keyboard-addressable',async()=>{await act(async()=>{root?.render(<App/>);await new Promise(resolve=>setTimeout(resolve,0));});const navigation=container.querySelector('nav[aria-label="Planning steps"]')!;const buttons=[...navigation.querySelectorAll('button')];expect(buttons.map(button=>button.textContent?.trim())).toEqual(['Goal','Explore','Plan','Activity']);for(const button of buttons){button.focus();expect(document.activeElement).toBe(button);}});
  it('lets a human select transport manually, the same way the agent tools do',async()=>{await act(async()=>{root?.render(<App/>);await new Promise(resolve=>setTimeout(resolve,0));});await act(async()=>{container.querySelector<HTMLButtonElement>('nav button:nth-child(2)')?.click();});const showtime=[...container.querySelectorAll<HTMLButtonElement>('.slot-row button')].find(button=>!button.disabled&&/\d{1,2}:\d{2}/.test(button.textContent??''));expect(showtime).toBeDefined();await act(async()=>{showtime?.click();});const chooseRestaurant=[...container.querySelectorAll('button')].find(button=>button.textContent==='Choose this restaurant');expect(chooseRestaurant).toBeDefined();await act(async()=>{chooseRestaurant?.click();});const transportButtons=[...container.querySelectorAll<HTMLButtonElement>('.transport-grid button')];expect(transportButtons.length).toBeGreaterThan(0);await act(async()=>{transportButtons[0].click();});expect(container.querySelector('.toast.error')).toBeNull();expect(transportButtons[0].className).toContain('selected');const saved=JSON.parse(localStorage.getItem('planonit.state.v5')??'{}');expect(saved.plan.selections.transportOptionId).toBeTruthy();});
});
