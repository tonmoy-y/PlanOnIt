...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const controller=new AbortController();const tools=buildTools(()=>planRef.current,commitExternal,demoProvider,authority);
  ...
  return <div className="app">
    ...
    <div className="sr-live" role={toast?.tone==='error'?'alert':'status'} aria-live={toast?.tone==='error'?'assertive':'polite'}>{toast?.text}</div>
...
function Explore({plan,snapshot,update}:{plan:Plan;snapshot:ReturnType<typeof snapshotPlan>;update:(change:Record<string,unknown>,text:string)=>Promise<MutationOutcome>}){
  ...
  return <><PageTitle eyebrow="BUILD YOUR EVENING" title="Film first, then dinner." body={`Planning for ${plan.people} ${plan.people===1?'person':'people'} on ${plan.date}. PlanOnIt books the table for after the film, using the same checks the agent tools use.`} status={snapshot.evaluation.valid?'Feasible':'Needs attention'}/>
    ...
    <section className="explore-section"><SectionTitle number="02" title="Dinner after the film" subtitle={currentShowtime?'PlanOnIt books the first table you can comfortably reach after the credits.':'Pick a film first — the table is booked around it.'}/><div className="card-grid">{restaurants.map(restaurant=>{
      ...
      <div className="choice-action">{chosen&&plan.selections.restaurantSlot?<span className="chosen-slot"><Check/> Table at {prettyTime(plan.selections.restaurantSlot)} for {plan.people}</span>:bookable?<span className="suggested-slot">Table around {prettyTime(best!.time)}</span>:<span className="unavailable">{!currentShowtime?'Choose a film first':ranked.some(item=>item.hasCapacity)?'No table fits after this film':'Fully booked for this group'}</span>}
      <button className="secondary" disabled={immutable||!bookable||chosen} onClick={()=>chooseRestaurant(restaurant.id,restaurant.name)}>{chosen?'Selected':'Choose this restaurant'}</button></div></article>})}</div></section>
...
function PlanView({plan,snapshot,approve,reserve,quickRepair,setTab,startNew,reservations,resetWorkspaceState}:{plan:Plan;snapshot:ReturnType<typeof snapshotPlan>;approve:()=>void;reserve:()=>void;quickRepair:()=>void;setTab:(tab:Tab)=>void;startNew:()=>void;reservations:Reservation[];resetWorkspaceState:()=>void}){
  ...
  // The evening runs movie → travel → dinner; the summary must never imply the reverse.
  return <><PageTitle eyebrow="CURRENT PLAN" title="Your evening, ready to verify." body={`${plan.date} · ${plan.people} ${plan.people===1?'person':'people'} · ${movie?.title??'A movie'}, then dinner at ${restaurant?.name??'a restaurant'}.`} status={statusLabel(plan,evaluation)}/>
    ...
    {!evaluation.valid&&immutable&&<section className="repair-callout"><div><Lock/><div><div className="eyebrow">{plan.status==='reserved'?'RESERVATION INTEGRITY':'RESERVATION IN FLIGHT'}</div><h2>{plan.status==='reserved'?'This reserved plan no longer matches its provider commitment.':'A reservation attempt is resolving.'}</h2><ul>{failed.map(item=><li key={item.id}><strong>{item.label}:</strong> {item.message}</li>)}</ul><p>{immutableReason(plan)}</p></div></div></section>}
    {!evaluation.valid&&!immutable&&<section className="repair-callout"><div><AlertTriangle/><div><div className="eyebrow">PLAN NEEDS ATTENTION</div><h2>{failed[0]?.message??'A blocking issue must be repaired.'}</h2><ul>{failed.slice(1).map(item=><li key={item.id}><strong>{item.label}:</strong> {item.message}</li>)}</ul><button className="text-button" onClick={()=>navigator.clipboard?.writeText(repairPrompt)}><Bot/> Copy agent repair prompt</button></div></div><div><button className="primary" onClick={quickRepair}><Wrench/> Repair plan now</button><button className="secondary" onClick={()=>setTab('explore')}><Pencil/> Fix manually</button></div></section>}
    <div className="plan-layout"><div className="plan-main"><section className="panel timeline"><div className="section-heading"><div><div className="eyebrow">ITINERARY</div><h2>{evaluation.timeline?'Film, travel, then dinner':'Complete the selections to calculate time'}</h2></div><Clock3/></div>{evaluation.timeline?<><TimelineRow icon={<Ticket/>} time={`${prettyTime(evaluation.timeline.movieStart)}–${prettyTime(evaluation.timeline.movieEnd)}`} title={movie?.title??'Movie'} detail={`${cinema?.name} · ${showtime?.inventoryState==='committed_to_current_plan'?`${showtime.committedQuantity} seats confirmed for you`:`${plan.people} tickets`}`}/><TimelineRow icon={<Navigation/>} time={`${prettyTime(evaluation.timeline.departAt)}–${prettyTime(evaluation.timeline.arriveAt)}`} title={`Travel · ${transport?.name??'Transport'}`} detail={`${route?.distanceKm} km · ${transport?.durationMinutes} min · ready to be seated by ${prettyTime(evaluation.timeline.readyAt)}`}/><TimelineRow icon={<Utensils/>} time={`${prettyTime(evaluation.timeline.dinnerStart)}–${prettyTime(evaluation.timeline.dinnerEnd)}`} title={restaurant?.name??'Restaurant'} detail={`${restaurant?.cuisine} · table for ${plan.people} · open ${restaurant?.openingHours}`}/></>:<Empty icon={<Clock3/>} title="No timeline yet" body="Restaurant, showtime, and transport are all required."/>}</section><section className="panel"><div className="section-heading"><div><div className="eyebrow">PLAN HEALTH</div><h2>{evaluation.valid?'Every blocking check passes':'Here is exactly what needs attention'}</h2></div><span className={`score-badge ${evaluation.valid?'pass':'fail'}`}>{evaluation.checks.filter(item=>item.passed).length}/{evaluation.checks.length}</span></div><CheckGrid checks={evaluation.checks}/></section>{reservations.length>0&&<section className="panel"><div className="section-heading"><div><div className="eyebrow">RESERVATION HISTORY</div><h2>Every sandbox commitment this workspace made</h2></div><ShieldCheck/></div><div className="reservation-history">{reservations.map(item=>{const current=item.planId===plan.id&&item.version===plan.version;return <div key={item.id} className={current?'active':'superseded'}><div><strong>{item.id}</strong><span>Plan version {item.version} · {item.inventory.filter(record=>record.state==='committed').length} inventory holds · {new Date(item.reservedAt).toLocaleString()}</span></div><span className={`reservation-tag ${item.status}`}>{item.status==='confirmed'?(current?'Active for this plan':'Still committed · superseded by a newer plan'):item.status==='failed'?`Failed · ${item.failureCode??'released'}`:'Pending'}</span></div>})}</div></section>}</div>
    <aside className="cost-card"><div className="eyebrow">TOTAL COST</div><h2>{money(evaluation.costs.total)}</h2><div className="budget-meter"><div className={evaluation.costs.remainingBudget!==null&&evaluation.costs.remainingBudget<0?'over':''} style={{width:`${evaluation.costs.total===null?0:Math.min(100,(evaluation.costs.total/plan.budget)*100)}%`}}/></div><div className="budget-row"><span>of {money(plan.budget)}</span><strong className={evaluation.costs.remainingBudget!==null&&evaluation.costs.remainingBudget<0?'danger':''}>{evaluation.costs.remainingBudget===null?'Incomplete':evaluation.costs.remainingBudget>=0?`${money(evaluation.costs.remainingBudget)} left`:`${money(Math.abs(evaluation.costs.remainingBudget))} over`}</strong></div><div className="breakdown"><CostRow label="Dinner" value={evaluation.costs.restaurant} detail={`${plan.people} × ${money(restaurant?.pricePerPerson)}`}/><CostRow label="Movie" value={evaluation.costs.movie} detail={`${plan.people} × ${money(showtime?.price)}`}/><CostRow label="Transport" value={evaluation.costs.transport} detail="route-specific fare"/></div><div className={`approval-note ${evaluation.valid?'pass':'fail'}`}>{evaluation.valid?<CheckCircle2/>:<ShieldCheck/>}<span>{plan.status==='reserved'?'Inventory is committed to this exact version.':plan.status==='approved'?'Approved. Confirm to commit sandbox inventory.':evaluation.valid?'Ready for your explicit approval.':'Approval and reservation are blocked.'}</span></div>{plan.status==='reserved'?<><div className="reservation-confirmed"><CheckCircle2/><strong>{plan.reservation?.id}</strong><span>{movie?.title} at {prettyTime(showtime?.startTime)}, then {restaurant?.name} at {prettyTime(plan.selections.restaurantSlot)}</span><span>{plan.date} · {plan.people} {plan.people===1?'person':'people'} · committed to version {plan.reservation?.version}</span></div><button className="primary full" onClick={startNew}><Sparkles/> Start a new plan</button><small className="fine-print">Starting a new plan keeps this reservation committed and listed below. It is never cancelled or hidden.</small></>:plan.status==='reservation_pending'?<button className="primary full" disabled><RefreshCw/> Confirming with the provider…</button>:plan.status==='approved'?<button className="primary full" onClick={reserve}><ShieldCheck/> Confirm sandbox reservation</button>:<button className="primary full" disabled={!evaluation.valid} onClick={approve}><ShieldCheck/> Approve this plan</button>}{!immutable&&<button className="text-button full" onClick={()=>setTab('explore')}><Pencil/> Edit choices</button>}<ResetControl plan={plan} reservations={reservations} resetWorkspaceState={resetWorkspaceState}/><small className="fine-print">A reservation commits controlled sandbox inventory. No real provider or payment is contacted.</small></aside></div>
  </>;
}
