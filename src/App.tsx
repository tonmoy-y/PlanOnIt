...
export default function App(){
  ...
  useEffect(()=>{
    ...
    Promise.all(registered.map(tool=>document.modelContext!.registerTool(tool,{signal:controller.signal}))).then(()=>setWebmcp('active')).catch(()=>setWebmcp('error'));
...
function Explore({plan,snapshot,update}:{plan:Plan;snapshot:ReturnType<typeof snapshotPlan>;update:(change:Record<string,unknown>,text:string)=>Promise<MutationOutcome>}){
  const currentShowtime=demoProvider.getShowtime(plan.selections.showtimeId);const currentCinema=demoProvider.getCinema(currentShowtime?.cinemaId);const currentRestaurant=demoProvider.getRestaurant(plan.selections.restaurantId);const route=demoProvider.getRoute(currentCinema?.locationId,currentRestaurant?.locationId);const shows=demoProvider.showtimesForDate(plan.date);
  ...
  return <><PageTitle eyebrow="BUILD YOUR EVENING" title="Film first, then dinner." body={`Planning for ${plan.people} ${plan.people===1?'person':'people'} on ${plan.date}. PlanOnIt books the table for after the film, using the same checks the agent tools use.`} status={snapshot.evaluation.valid?'Feasible':'Needs attention'}/>
    ...
    <section className="explore-section"><SectionTitle number="03" title="Getting there" subtitle="How you travel from the cinema to the restaurant. Only route-backed options appear."/>{route?<div className="route-card"><div className="route-summary"><RouteIcon/><div><strong>{currentCinema?.name.split(',')[0]??demoProvider.getLocation(route.fromLocationId)?.name} → {currentRestaurant?.name??demoProvider.getLocation(route.toLocationId)?.name}</strong><span>{route.distanceKm} km after the film</span></div></div><div className="transport-grid">{route.options.map(option=><button disabled={immutable} className={plan.selections.transportOptionId===option.id?'selected':''} key={option.id} onClick={()=>update({transportOptionId:option.id},`Selected ${option.name}`)}><Navigation/><strong>{option.name}</strong><span>{option.durationMinutes} min · {money(option.fare)}</span><small>{option.description}</small></button>)}</div></div>:<Empty icon={<RouteIcon/>} title="Choose a film and a restaurant first" body="Then PlanOnIt can resolve a real route between the two venues."/>}</section>
  </>;
}
