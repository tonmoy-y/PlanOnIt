...
export const planSchema=z.object({
  ...
  reservation:z.object({id:z.string(),planId:z.string(),version:z.number().int().positive(),providerRevision:z.number().int().nonnegative(),status:z.enum(['pending','confirmed','failed']),reservedAt:z.string(),idempotencyKey:z.string(),inventory:z.array(z.object({kind:z.enum(['restaurant','showtime']),inventoryKey:z.string(),quantity:z.number().int().positive(),state:z.enum(['held','committed','released'])}).strict()),failureCode:z.string().optional()}).strict().optional()
}).strict();
